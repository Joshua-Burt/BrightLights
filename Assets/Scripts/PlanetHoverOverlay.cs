    ...
    private void Update() {
        if(_cam is { } && clone) {
            ...
            if(pos.x > 0 && pos.x < Screen.width && pos.y > 0 && pos.y < Screen.height && 
               (_cam.transform.position - _transform.position).magnitude > GetComponent<ShapeSettings>().planetRadius * 9) {
                clone.SetActive(true);
                
                float fadingValue = Math.Abs(1000 / distance(pos.x, pos.y, Screen.width / 2f, Screen.height / 2f));
